/*
 * A modified version of the TwoWayGridView for recent Android versions
 *
...
package thirdparty.two_way_grid_view;
...
public abstract class TwoWayAbsListView extends TwoWayAdapterView<ListAdapter> implements
    ...
    public TwoWayAbsListView(Context context, AttributeSet attrs, int defStyle) {
        ...
        mScrollVerticallyPortrait = false;
        mScrollVerticallyLandscape = false;
    ...
    private void useDefaultSelector() {
        setSelector(ResourcesUtils.getDrawable(
    ...
    protected float getTopFadingEdgeStrength() {
        ...
        if (count == 0 || !mScrollVertically) {
            ...
            return top < paddingTop ? -(top - paddingTop) / fadeLength : fadeEdge;
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        mListPadding.left = mSelectionLeftPadding + getPaddingLeft();
        mListPadding.top = mSelectionTopPadding + getPaddingTop();
        mListPadding.right = mSelectionRightPadding + getPaddingRight();
        mListPadding.bottom = mSelectionBottomPadding + getPaddingBottom();
    }
    ...
    public void setSelector(int resID) {
        setSelector(ResourcesUtils.getDrawable(resID));
    }
    ...
    /**
     * A base class for Runnable instances that will check that their view is still attached to
     ...
     * self evident by the relationship between the rect instances...
     *
    ...
    /**
     * Sets the direction that the view should scroll when in portrait orientation
     *
     ...
     *
     * @return {@link #SCROLL_AXIS_VERTICAL} or {@link #SCROLL_AXIS_HORIZONTAL}
     */
    public int getScrollDirectionPortrait() {
        return mScrollVerticallyPortrait ? SCROLL_AXIS_VERTICAL : SCROLL_AXIS_HORIZONTAL;
    }

    /**
     * Sets the direction that the view should scroll when in landscape orientation
     *
     * @param direction the view should scroll
     *
     * @see #SCROLL_AXIS_VERTICAL
    ...
    public int getScrollDirectionLandscape() {
        return mScrollVerticallyLandscape ? SCROLL_AXIS_VERTICAL : SCROLL_AXIS_HORIZONTAL;
    }
    ...
    protected boolean checkConsistency(int consistency) {
        ...
        if (doNotCheckScrap(scrap)) result = false;
        final ArrayList<View>[] scraps = mRecycler.mScrapViews;
        count = scraps.length;
        for (int i = 0; i < count; i++) {
            if (doNotCheckScrap(scraps[i])) result = false;
        }

        return result;
    }

    private boolean doNotCheckScrap(ArrayList<View> scrap) {
    ...
    abstract class TouchHandler {
        ...
        protected abstract class FlingRunnable implements Runnable {
            ...
            public boolean isScrollingInDirection(float xVelocity, float yVelocity) {
                final int dx = mScroller.getFinalX() - mScroller.getStartX();
                final int dy = mScroller.getFinalY() - mScroller.getStartY();
                return !mScroller.isFinished() && Math.signum(xVelocity) == Math.signum(dx) &&
                        Math.signum(yVelocity) == Math.signum(dy);
            }
    ...
    class VerticalTouchHandler extends TouchHandler {
        ...
        boolean trackMotionScroll(int deltaY, int incrementalDeltaY) {
            if (DEBUG) Log.i(TAG, "trackMotionScroll() - deltaY: " + deltaY + " incrementalDeltaY: " + incrementalDeltaY);
            final int childCount = getChildCount();
            if (childCount == 0) {
                return true;
            }
            ...
            final int footerViewsStart = mItemCount;
                ...
                if(mCheckFlywheel == null) {
                    mCheckFlywheel = new Runnable() {
                        public void run() {
                            ...
                            final float yVelocity = -vt.getYVelocity();

                            if (Math.abs(yVelocity) >= mMinimumVelocity
                                    && isScrollingInDirection(0, yVelocity)) {
    ...
    class HorizontalTouchHandler extends TouchHandler {
        ...
        boolean trackMotionScroll(int delta, int incrementalDelta) {
            if (DEBUG) Log.i(TAG, "trackMotionScroll() - deltaX: " + delta + " incrementalDeltaX: " + incrementalDelta);
            final int childCount = getChildCount();
            if (childCount == 0) {
                return true;
            }
                ...
                if(mCheckFlywheel == null) {
                    mCheckFlywheel = new Runnable() {
                        public void run() {
                            ...
                            vt.computeCurrentVelocity(1000, mMaximumVelocity);
                            final float xVelocity = -vt.getXVelocity();

                            // noinspection all
                            if (Math.abs(xVelocity) >= mMinimumVelocity
                                    && isScrollingInDirection(0, xVelocity)) {
                                // Keep the fling alive a little longer
                                postDelayed(this, FLYWHEEL_TIMEOUT);
                            } else {
                                endFling();
